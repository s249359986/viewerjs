...
export default {
   ...
   * @returns {Viewer} this
   */
  view(index = this.options.initialViewIndex,force = false) {
  ...
  updateItem(image,index){
    if(this.ready){
      this.updateItemProxy(image,index)
    }else{
      setTimeout(()=>{
        this.updateItem(image,index)
      },10)
    }
  },
  updateItemProxy(image,index) { // 更新
    ...
    if (src || url) {
      ...
      forEach(this.options.inheritedAttributes, (name) => {
      ...
      tempList.forEach((child,childIndex)=>{
        if(index === childIndex){
          ulSelector.replaceChild(item,child)
          if(this.index == index){
            this.view(this.index,true);
          }
        }
      })
    }
